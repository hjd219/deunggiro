...
UA={'User-Agent':'Mozilla/5.0 (compatible; DeunggiroBlogImporter/1.8; +https://www.deunggiro.kr/)'}
def get(url):
...
def table_html(table):
 """네이버 표의 행/열, colspan/rowspan, 셀 안의 기본 서식을 그대로 보존한다."""
 rows=[]
 for tr in table.find_all('tr'):
  ...
  for cell in tr.find_all(['th','td'],recursive=False):
   ...
   tag='th' if cell.name=='th' else 'td';attr=(' '+ ' '.join(attrs)) if attrs else ''
