...
UA={'User-Agent':'Mozilla/5.0 (compatible; DeunggiroBlogImporter/1.2; +https://www.deunggiro.kr/)'}
...
def norm_title(v):
 v=html.unescape(v or '');v=re.sub(r'^\s*\[[^\]]+\]\s*','',v);return re.sub(r'[^0-9A-Za-z가-힣]+','',v).lower()
...
def resolve_post_view(url):
 ...
 return f'https://blog.naver.com/PostView.naver?blogId={BLOG_ID}&logNo={n}&redirect=Dlog&widgetTypeCall=true&directAccess=false' if n else url

def heading_like(txt):
 """네이버에서 일반 문단으로 저장된 1. 2. 3. / 1️⃣ 2️⃣ 제목도 홈페이지 대제목으로 변환."""
 t=re.sub(r'\s+',' ',txt).strip()
 return bool(re.match(r'^(?:[1-9]|1[0-9])[.)]\s*\S|^[1-9]️⃣\s*\S|^(?:①|②|③|④|⑤|⑥|⑦|⑧|⑨|⑩)\s*\S',t)) and len(t)<=90
...
def looks_mojibake(text):
 ...
 return sum(text.count(m) for m in ('ë','ì','í','â€','Â','�'))>=4

def refresh_existing_imports(posts):
 """기존 네이버 자동수집 글도 짧은 요약/제목 구분/공통 템플릿으로 다시 생성."""
 refreshed=0
 for post in posts:
...
def main():
 ...
 for item in fetch_feed_items():
  ...
  post={'title':item['title'],'category':infer_category(item['title']+' '+text[:500]),'date':item['date'],'slug':slug,'keywords':item['title'],'summary':summary_from(text,item['title']),'source_url':item['link'],'source':'naver-blog'}
  (POSTS_DIR/f'{slug}.html').write_text(build_html(post,body),encoding='utf-8');posts.insert(0,post);existing_titles.add(norm_title(post['title']));existing_sources.add(item['link']);imported+=1;print('IMPORTED',slug,post['title'])
