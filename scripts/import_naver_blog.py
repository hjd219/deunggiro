...
def norm_title(v):
    ...
    v = re.sub('^\\s*\\[[^\\]]+\\]\\s*', '', v)
    return re.sub('[^0-9A-Za-z가-힣]+', '', v).lower()
...
def resolve_post_view(url):
    ...
    n = log_no_from_url(url)
    if n:
        return f'https://blog.naver.com/PostView.naver?blogId={BLOG_ID}&logNo={n}&redirect=Dlog&widgetTypeCall=true&directAccess=false'
    return url
...
def save_image(src, slug, index):
    try:
        ...
        path = folder / f'{index:02d}{image_ext(r, src)}'
        path.write_bytes(r.content)
        return '/' + path.relative_to(ROOT).as_posix()
    except Exception as e:
        print('IMAGE_SKIP', src, e, file=sys.stderr)
        return src
...
def is_thumbnail_image(el):
    ...
    for _ in range(5):
        ...
        if any((x in marker for x in THUMBNAIL_MARKERS)):
            return True
        node = node.parent
    return False
...
def has_substantive_text_after(blocks, index):
    for nxt in blocks[index + 1:]:
        ...
        if not txt:
            continue
        if any((marker in txt for marker in FOOTER_IMAGE_MARKERS)):
            continue
        return True
    return False
...
def clean_article(url, slug=''):
    ...
    for idx, el in enumerate(blocks):
        ...
        if txt and any((marker in txt for marker in FOOTER_IMAGE_MARKERS)):
            footer_zone = True
        if el.name == 'img':
            if footer_zone or is_thumbnail_image(el) or (not has_substantive_text_after(blocks, idx)):
                continue
            ...
            if slug:
                image_no += 1
                src = save_image(src, slug, image_no)
    ...
    body = '\n'.join(parts)
    text = re.sub('\\s+', ' ', ' '.join(root.stripped_strings)).strip()
    return (body, text)

def summary_from(text, title):
    clean = re.sub('^\\s*\\[[^\\]]+\\]\\s*', '', html.unescape(title or '')).strip()
    clean = re.sub('\\s+', ' ', clean)
    if len(clean) > 42:
        clean = clean[:42].rstrip(' ,·:-')
...
def build_html(post, body):
    ...
    intro_title = re.sub('^\\s*\\[[^\\]]+\\]\\s*', '', post['title'])
    intro = html.escape(f'{intro_title}에서 꼭 확인해야 할 핵심 내용을 순서대로 살펴보겠습니다.')
...
def normalize_list_entry(raw):
    ...
    if not re.fullmatch('\\d{6,}', n):
        return None
    title = raw.get('title') or raw.get('postTitle') or raw.get('subject') or ''
    if isinstance(title, dict):
        title = title.get('value') or title.get('text') or ''
    title = BeautifulSoup(html.unescape(str(title)), 'html.parser').get_text(' ', strip=True)
    date_value = raw.get('addDate') or raw.get('postAddDate') or raw.get('writeDate') or raw.get('regDate') or raw.get('date') or ''
...
def fetch_all_blog_items():
    ...
    for page in range(1, MAX_LIST_PAGES + 1):
        try:
            r = get(TITLE_LIST_URL, params={'blogId': BLOG_ID, 'viewdate': '', 'currentPage': page, 'categoryNo': 0, 'parentCategoryNo': '', 'countPerPage': LIST_PAGE_SIZE})
            payload = decode_title_list_response(r.text)
            items = extract_entries_from_payload(payload)
        except Exception as e:
            print('LIST_PAGE_SKIP', page, e, file=sys.stderr)
            break
        if not items:
            consecutive_empty += 1
        ...
        if len(items) < LIST_PAGE_SIZE:
            break
    if out:
        print(f'LIST_TOTAL={len(out)}')
        return out
    print('LIST_FALLBACK_RSS', file=sys.stderr)
    return fetch_feed_items()

def looks_mojibake(text):
    return bool(text) and sum((text.count(m) for m in ('ë', 'ì', 'í', 'â€', 'Â', '�'))) >= 4
...
def refresh_existing_imports(posts):
    ...
    for post in posts:
        ...
        try:
            body, text = clean_article(post['source_url'], slug)
        except Exception as e:
            print('REFRESH_SKIP', slug, e, file=sys.stderr)
            continue
...
def main():
    ...
    posts = load_posts()
    refreshed = refresh_existing_imports(posts)
    ...
    for p in posts:
        n = log_no_from_url(str(p.get('source_url', '')))
        if not n:
            m = re.fullmatch('naver-(\\d{6,})', str(p.get('slug', '')).replace('.html', ''))
            n = m.group(1) if m else ''
        if n:
            existing_log_nos.add(n)
    imported = 0
    candidates = fetch_all_blog_items()
    missing = []
    for item in candidates:
        n = item.get('log_no') or log_no_from_url(item['link'])
        if not n or n in existing_log_nos:
            continue
        if item['link'] in existing_sources:
            continue
        if item.get('title') and norm_title(item['title']) in existing_titles:
            continue
        missing.append(item)
    print(f'CANDIDATES={len(candidates)} MISSING={len(missing)}')
    for item in missing:
        ...
        slug = f'naver-{n}'
        try:
            body, text = clean_article(item['link'], slug)
        except Exception as e:
        ...
        if not title:
            title = f'네이버 블로그 글 {n}'
        post = {'title': title, 'category': infer_category(title + ' ' + text[:500]), 'date': item.get('date') or datetime.now().strftime('%Y-%m-%d'), 'slug': slug, 'keywords': title, 'summary': summary_from(text, title), 'source_url': canonical_blog_url(n), 'source': 'naver-blog'}
        (POSTS_DIR / f'{slug}.html').write_text(build_html(post, body), encoding='utf-8')
        posts.insert(0, post)
        existing_titles.add(norm_title(title))
        existing_sources.add(post['source_url'])
        existing_log_nos.add(n)
        imported += 1
