(()=>{
  ...
  let data=[],mode='all',query='',page=1;
  ...
  function render(){
    const list=data.filter(matches),posts=document.getElementById('posts'),pagination=document.getElementById('pagination');if(!posts)return;
    const totalPages=Math.max(1,Math.ceil(list.length/PAGE_SIZE));if(page>totalPages)page=totalPages;
    const shown=list.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);
    posts.className='post-grid dg-legal-scroll';posts.innerHTML=shown.length?shown.map(card).join(''):'<div class="dg-no-result">검색 결과가 없습니다.</div>';
    let head=document.getElementById('dg-legal-result-head');if(!head){head=document.createElement('div');head.id='dg-legal-result-head';head.className='dg-legal-result-head';posts.before(head)}
    const label=TAB_DEFS.find(x=>x[1]===mode)?.[0]||'전체';head.innerHTML=`<strong>${esc(query?'검색 결과':label)}</strong><span>${list.length}개 글${totalPages>1?` · ${page}/${totalPages} 페이지`:''}</span><div class="dg-scroll-arrows"><button type="button" data-dir="-1" aria-label="이전">‹</button><button type="button" data-dir="1" aria-label="다음">›</button></div>`;
    ...
    if(pagination){pagination.innerHTML=totalPages>1?Array.from({length:totalPages},(_,i)=>`<button class="page-btn ${i+1===page?'active':''}" type="button" data-page="${i+1}">${i+1}</button>`).join(''):'';pagination.querySelectorAll('button').forEach(b=>b.onclick=()=>{page=Number(b.dataset.page);render();posts.scrollLeft=0;document.querySelector('.post-toolbar')?.scrollIntoView({behavior:'smooth',block:'start'})})}
  }
