document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(current==='/posts.html'){
    const preferred=['전체','상속등기','상속포기·한정승인','상속재산분할','법인등기','부동산등기','가사'];
    const reorderCats=()=>{
      const cats=document.getElementById('cats'); if(!cats)return;
      const buttons=[...cats.querySelectorAll('.chip')]; if(buttons.length<2)return;
      const byText=new Map(buttons.map(b=>[b.textContent.trim(),b]));
      preferred.forEach(name=>{const b=byText.get(name);if(b)cats.appendChild(b)});
      buttons.filter(b=>!preferred.includes(b.textContent.trim())).forEach(b=>cats.appendChild(b));
    };
    reorderCats();
    const cats=document.getElementById('cats'); if(cats)new MutationObserver(reorderCats).observe(cats,{childList:true});
  }
