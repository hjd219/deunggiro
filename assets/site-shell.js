document.addEventListener('DOMContentLoaded',()=>{
  ...
  /* HEADER_FOOTER_RESTORE_V1: 기존 페이지가 가진 헤더는 보존 */
  const existingHeader=document.querySelector('header.header,header.dg-shell-header');
  if(existingHeader){
    const hasOwnHeader=existingHeader.classList.contains('header') && existingHeader.children.length>0;
    if(!hasOwnHeader) existingHeader.outerHTML=header;
  }else{
  ...
  /* HEADER_FOOTER_RESTORE_V1: 기존 페이지가 가진 푸터도 보존 */
  const existingFooter=document.querySelector('footer.footer,footer.dg-shell-footer');
  if(existingFooter){
    const hasOwnFooter=existingFooter.classList.contains('footer') && existingFooter.children.length>0;
    if(!hasOwnFooter) existingFooter.outerHTML=footer;
  }else{
