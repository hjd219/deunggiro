...
const fixed=[
 {title:'법인등기 한눈에 보기',href:'#documents',type:'메인안내'},
 ...
 {title:'1인 법인 설립 처리사례',href:'/posts/naver-224411175706.html',type:'처리사례'},
 {title:'대표이사 사망 후 변경 처리사례',href:'/posts/naver-224411086807.html',type:'처리사례'},
 {title:'감사 중임·의결권 3% 처리사례',href:'/posts/naver-224411143025.html',type:'처리사례'},
 {title:'본점 관외이전 처리사례',href:'/posts/naver-224411109357.html',type:'처리사례'},
 {title:'회사계속·부활등기 처리사례',href:'/posts/naver-224411074752.html',type:'처리사례'}
]()=>{'use strict';
const q=document.getElementById('inheritance-query'),list=document.getElementById('inheritance-results'),toggle=document.getElementById('inheritance-toggle'),go=document.getElementById('inheritance-search-button');
if(!q||!list||!toggle||!go)return;
const fixed=[
...
let items=[...fixed];
fetch('/assets/posts.json').then(r=>r.ok?r.json():[]).then(data=>{if(Array.isArray(data))items=items.concat(data.filter(p=>p&&p.category==='법인등기').map(p=>({title:p.title||'',href:p.href||p.url||'',type:'처리사례'})).filter(x=>x.title&&x.href&&!items.some(y=>y.href===x.href)));}).catch(()=>{});
