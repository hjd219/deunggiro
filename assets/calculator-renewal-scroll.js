(function(){
 const stage=document.querySelector('.v53-stage'),win=document.querySelector('.v53-window'),track=document.querySelector('.v53-track'),page=track&&track.querySelector('.calc-page-shell');
 ...
 function measure(){
  ...
  const wh=win.clientHeight,pageTop=page.offsetTop,pageH=page.offsetHeight,isMobile=matchMedia('(max-width:800px)').matches;
  const bottomGap=isMobile?20:Math.max(16,Math.min(64,wh-pageH-16));
  /* Long calculator pages must keep moving until their REAL bottom reaches the footer gap. */
  finalY=wh-bottomGap-(pageTop+pageH);
 ...
 function update(){
  const wh=win.clientHeight,p=Math.max(0,(scrollY||0)-start);
  ...
  /* Pin only after the visible calculator bottom has reached the target gap. */
