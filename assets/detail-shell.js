(()=>{
  ...
  const faqData={
    ...
    '/inheritance-substitute-succession.html':[['대습상속은 언제 발생하나요?','원래 상속인이 될 사람이 피상속인보다 먼저 사망한 경우 등 법정요건을 확인합니다.'],['대습상속인의 배우자도 상속인이 되나요?','혼인관계와 사망시점 등 법정요건을 함께 확인해야 합니다.'],['대습상속 지분은 어떻게 계산하나요?','피대습자가 받을 상속분을 기준으로 대습상속인들의 지분을 계산합니다.']],
    '/inheritance-division.html':[['상속인 전원이 합의하지 못하면 어떻게 하나요?','가정법원의 상속재산분할심판을 통해 분할방법을 정하는 절차를 검토할 수 있습니다.'],['한 명이 부동산을 받고 다른 상속인에게 돈을 줄 수 있나요?','협의 또는 심판에서 특정 상속인의 단독취득과 정산금 지급 방식이 가능합니다.'],['연락두절 상속인이 있어도 분할심판이 가능한가요?','상속인 전원을 당사자로 특정하여 법원절차를 진행하는 방안을 검토할 수 있습니다.']]};
  const rows=faqData[PATH];if(rows){let s=[...document.querySelectorAll('article .section')].find(x=>x.querySelector('.kicker')?.textContent.trim()==='FAQ');if(!s){s=document.createElement('section');s.className='section';document.querySelector('article')?.appendChild(s)}s.innerHTML=`<div class="faq-kicker">FAQ</div><h2>자주 묻는 질문</h2><div class="faq"><table class="faq-table"><tbody>${rows.map(r=>`<tr><th>${r[0]}</th><td>${r[1]}</td></tr>`).join('')}</tbody></table></div>`}

  const items=[['연락두절 상속인이 있는 경우','/inheritance-missing-heir.html'],['해외거주·외국인 상속인이 있는 경우','/inheritance-overseas-heir.html'],['미성년 상속인이 있는 경우','/inheritance-minor-heir.html'],['대습상속','/inheritance-substitute-succession.html'],['상속재산분할이 필요한 경우','/inheritance-division.html']];
  const trigger=[...document.querySelectorAll('.actions a,.actions button')].find(x=>x.textContent.includes('상속등기 세부안내'));
  if(trigger){trigger.removeAttribute('href');trigger.setAttribute('role','button');trigger.setAttribute('aria-haspopup','menu');trigger.setAttribute('aria-expanded','false');trigger.classList.add('dg-detail-pop-trigger');trigger.innerHTML='상속등기 세부안내 <span class="dg-detail-trigger-arrow" aria-hidden="true"></span>';const pop=document.createElement('div');pop.className='dg-detail-popover';pop.setAttribute('role','menu');pop.setAttribute('aria-hidden','true');pop.innerHTML=`<div class="dg-detail-pop-title">상속등기 세부안내</div>${items.map(([label,href])=>`<a href="${href}" role="menuitem" class="${href===PATH?'active':''}">${label}<span>›</span></a>`).join('')}`;document.body.appendChild(pop);const pos=()=>{if(!pop.classList.contains('open'))return;const r=trigger.getBoundingClientRect(),w=pop.offsetWidth||360;let left=Math.max(10,Math.min(r.left,window.innerWidth-w-10));let top=r.bottom+10;const ph=pop.offsetHeight||280;if(top+ph>window.innerHeight-10&&r.top-ph-10>10)top=r.top-ph-10;pop.style.left=left+'px';pop.style.top=top+'px'};const open=v=>{pop.classList.toggle('open',v);pop.setAttribute('aria-hidden',v?'false':'true');trigger.setAttribute('aria-expanded',v?'true':'false');if(v)requestAnimationFrame(pos)};trigger.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();open(!pop.classList.contains('open'))});pop.addEventListener('click',e=>e.stopPropagation());document.addEventListener('click',()=>open(false));document.addEventListener('keydown',e=>{if(e.key==='Escape')open(false)});window.addEventListener('resize',pos);window.addEventListener('scroll',pos,true)}
})();