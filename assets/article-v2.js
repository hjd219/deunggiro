document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(oldHeader) oldHeader.outerHTML=`<header class="header"><div class="container header-inner"><a class="logo" href="/"><span>등기로</span><small>현재두 법무사 사무소 · 인천</small></a><nav class="nav"><a href="/">홈</a><a href="/inheritance.html">상속등기</a><a href="/renunciation.html">상속포기·한정승인</a><a href="/corporate.html">법인등기</a><a href="/realestate.html">부동산등기</a><a href="/family.html">가사</a><a href="/posts.html" aria-current="page">법률정보</a></nav><button type="button" class="mobile-menu-btn" id="article-mobile-menu-btn" aria-expanded="false">☰ 메뉴</button><a class="mobile-only" href="tel:0324251500">전화상담</a></div></header><div class="mobile-menu-panel" id="article-mobile-menu-panel" aria-hidden="true"><div class="mobile-menu-grid"><a href="/">홈</a><a href="/inheritance.html">상속등기</a><a href="/renunciation.html">상속포기·한정승인</a><a href="/corporate.html">법인등기</a><a href="/realestate.html">부동산등기</a><a href="/family.html">가사</a><a href="/posts.html">법률정보</a></div><button type="button" class="mobile-menu-close" id="article-mobile-menu-close">메뉴 닫기 ↑</button></div>`;

  const oldCta=document.querySelector('section.cta');
  if(oldCta) oldCta.outerHTML=`<section class="contact" id="contact"><div class="container contact-grid"><div><div class="label">CONSULTATION</div><h2>복잡한 등기절차, 등기로에서 확인하세요.</h2><p>상속 · 법인 · 부동산 등 필요한 절차와 준비서류를 확인할 수 있습니다.</p></div><a class="phone" href="tel:0324251500">032-425-1500</a></div></section>`;

  const oldFooter=document.querySelector('footer.footer');
  if(oldFooter) oldFooter.outerHTML=`<footer class="footer"><div class="container footer-route-grid"><div><div class="footer-brand">등기로</div><div class="footer-office">현재두 법무사 사무소</div><div class="footer-info"><div><strong>주소</strong> 인천 미추홀구 경원대로 873, 201호(주안동, 인성빌딩) · 인천가정법원 옆</div><div><strong>전화</strong> <a href="tel:0324251500" style="color:#fff;font-weight:800">032-425-1500</a></div></div></div><div class="footer-route"><div class="footer-route-title">찾아오시는 길</div><div class="footer-route-sub">인천가정법원 옆 · 인성빌딩 2층</div><div class="footer-route-list"><div><span>🚇</span><span><b>1호선</b> 주안역·간석역 1번 출구 도보 이용</span></div><div><span>🚇</span><span><b>인천지하철 2호선</b> 석바위시장역 하차 후 석바위 지하상가 5번 출구 도보 이용</span></div></div></div></div></footer>`;

  const btn=document.getElementById('article-mobile-menu-btn'),panel=document.getElementById('article-mobile-menu-panel'),close=document.getElementById('article-mobile-menu-close');
  if(btn&&panel){const setOpen=open=>{panel.classList.toggle('open',open);panel.setAttribute('aria-hidden',open?'false':'true');btn.setAttribute('aria-expanded',open?'true':'false')};btn.addEventListener('click',()=>setOpen(!panel.classList.contains('open')));close?.addEventListener('click',()=>setOpen(false));panel.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>setOpen(false)));document.addEventListener('keydown',e=>{if(e.key==='Escape')setOpen(false)})}

  const escText=v=>String(v||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
  const groupOf=c=>{c=String(c||'');if(c.includes('상속'))return '상속';if(c.includes('법인'))return '법인';if(c.includes('부동산'))return '부동산';if(c.includes('가사'))return '가사';return c||'기타'};
  const listLabel=g=>g==='상속'?'상속 법률정보 목록':g==='법인'?'법인 법률정보 목록':g==='부동산'?'부동산 법률정보 목록':g==='가사'?'가사 법률정보 목록':'법률정보 목록';
